...
function Input({ label, placeholder, type = "text", value, onChange }) {
  ...
  return (
    <div className="relative mt-4">
      ...
      />
      <label
        className={`bg-white px-1 absolute left-2 -top-3.5 text-gray-600 transition-all duration-300 transform origin-[0] ${
      ...
      {type === "password" && (
        <div className="absolute right-5 top-1/2 -translate-y-1/2">
          {isPasswordVisible ? (
            <EyeSlashIcon
              className="h-6 w-6 cursor-pointer text-slate-500 hover:text-slate-700 transition-colors duration-200"
